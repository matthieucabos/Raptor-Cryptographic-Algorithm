...
def crypto_procedure():
	"""
		...
		The alorithm is ruled by the following steps :
			...
			* Once each slices properly cutted, we have to crypt each of them using the crypt_procedure automated on a loop coursing each of them.
			* The crypted slices are managed via a second level separators set wich define a second level of crypting tree. In fact each term of a slice is using a first level of separators, it give a one-level tree. The second level permit to complexify the full algorithm result.
			* Finally, the crypt_procedure function is used to associate each crypted slice to his key and draw a correct interpretated result as list of couple (crypted string/integer key)
			* The couple full result rebuilded from slices couple is organized from the second level separators to draw a 2-level tree 
